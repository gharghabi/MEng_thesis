...
void calc_correspondense( pcl::PointCloud<pcl::VFHSignature308>::Ptr VFHfeature,const std::string &filename)
{
    ...
    float minDist[rank_k] = {10000,10000,10000,10000,10000};
    string minDistCloudName[rank_k];
...
//            cout<<"203"<<endl;
//            for(int y=0;y<rank_k;++y)
//            cout<<minDist[y]<<" dist";
//            cout<<endl;
            if(model_scene_corrs->at(0).distance<minDist[rank_k-1])
            {

//                cout<<"206"<<endl;
                minDist[rank_k-1] = model_scene_corrs->at(0).distance;
                minDistCloudName[rank_k-1] = strsModel.at((strsModel.size()-1));
                int q=rank_k-1;
                while((minDist[q]<minDist[q-1])&&(q>0))
                {
//                    cout<<q<<" q 213"<<endl;
                    float tempDist = minDist[q-1];
                    string tempName = minDistCloudName[q-1];
                    minDist[q-1] = minDist[q];
                    minDistCloudName[q-1] = minDistCloudName[q];
                    minDist[q] = tempDist;
                    minDistCloudName[q] = tempName;
                    --q;
                }
            }
    ...
    {
        if((minDistCloudName[q].at(0)==normal_path.at(9))&&(minDistCloudName[q].at(1)==normal_path.at(10)))
        {
            ++correctDetect;
            break;
        }
