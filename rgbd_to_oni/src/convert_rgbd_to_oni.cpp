...
// //         resize(matFrameImage, matFrameImage, Size(480,640), 0, 0, INTER_CUBIC);
...
{
    DepthMap& depthMap = depthMD.WritableDepthMap();
    for (XnUInt32 y = 0; y < (depthMap.YRes())/2; y++)
    {
        for (XnUInt32 x = 0; x < (depthMap.XRes())/2; x++)
        {
            //Punch vertical cut lines in the depth image
//            if ((x % 2) == 0)
//            {
...
int main(int argc, char* argv[])
{
    ...
    nRetVal = recorder.Create(context);
    CHECK_RC(nRetVal, "Create recorder");
    nRetVal = recorder.SetDestination(XN_RECORD_MEDIUM_FILE, "/home/shaghayegh/ddata1_0512164529_size320.oni");
    CHECK_RC(nRetVal, "Set recorder destination file");
    // add depth node to recorder
    nRetVal = recorder.AddNodeToRecording(mockDepth);
    CHECK_RC(nRetVal, "Add node to recording");
