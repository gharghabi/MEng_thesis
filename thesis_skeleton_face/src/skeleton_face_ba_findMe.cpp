...
void loadtestModels ()
{
    int percentage[featuresCount+1];
    ...
    for(int i = 0; i<dataNum; ++i)
    {
        int frameNum = readDataSetClient->set_parameters((i+1),activityName[l]);
        Rect result;
        for(int j = 1; j<(frameNum+1); ++j)
        {
            ++allFrameNum;
            ...
            {
                (&findMe[r])->updateImage(RGBImage);
                (&findMe[r])->doWork();
                conf[r] = (&findMe[r])->getRect(result);
                if(conf[r]>MaxConf)
                {
                    maxConf = conf[r];
                    maxConfIndex = r;
    ...
    OFTestData<<" recogntion percentae activitye : "<<activityName[l]<<"\n";
    for(int q = 0; q<featuresCount+1; ++q)//akaharin vijegi baraye tld mibashad
    {
        ...
        OFTestData<<" all frame num : "<<allFrameNum<<" number if correct detection: "<<percentage[q]<<" feature: "<<q<<" percentage "<<percentage[q]<<"\n";
